<div class="modal fade" id="modal-observar">
    <div class="modal-dialog">
        <div class="modal-content">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <div>
                                        <label>
                                            <input type="radio" name="descricao" value="Texto"> Modelo 3
                                        </label>
                                    </div>
                                    <div>
                                        <label>
            ...
            <div class="modal-footer justify-content-between">
                ...
                <button id="btn-salvar-observar" type="button" class="btn btn-primary">Salvar</button>
            </div>
        </div>
    </div>
</div>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        radioButtons.forEach(function(radioButton) {
            radioButton.addEventListener('change', function() {
                if (this.value === '') {
                    descricaoPersonalizada.removeAttribute('disabled');
                    descricaoPersonalizada.value = '';
                } else {
                    descricaoPersonalizada.value = this.value;
                    descricaoPersonalizada.setAttribute('disabled', 'disabled');
                }
            });
        });

        const btnSalvarObservar = document.getElementById('btn-salvar-observar');

        btnSalvarObservar.addEventListener('click', function() {
            const descricaoSelecionada = document.querySelector('input[name="descricao"]:checked').value;
            const observarTextarea = document.getElementById('descricao-personalizada');

            if (descricaoSelecionada !== '') {
                observarTextarea.value = descricaoSelecionada;
                observarTextarea.setAttribute('disabled', 'disabled');
            }
        });
    });
</script>