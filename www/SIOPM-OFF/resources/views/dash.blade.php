...
@section('content')
    ...
    <script>
        ...
        const despachadorSubMenu = '<div class="mb-3">' +
            '<label for="exampleInputTipoServico" class="form-label">Escolha o tipo de serviço</label>' +
            '<select class="form-select" name="tipoServico" id="exampleInputTipoServico" required>' +
            '<option value="" disabled selected>Escolha uma opção</option>' +
            '<option value="radioPatrulhamento">Radio Patrulhamento</option>' +
            '<option value="especialidades">Especialidades</option>' +
            '</select>' +
            '</div>';

        const radioPatrulhamentoSubMenu = '<div class="mb-3">' +
        ...
        const especialidadesSubMenu = '<div class="mb-3">' +
            '<label for="exampleInputEspecialidades" class="form-label">Especialidades</label>' +
            '<select class="form-select" name="especialidade" id="exampleInputEspecialidades" required>' +
            '<option value="" disabled selected>Escolha uma opção</option>' +
        ...
        document.getElementById('exampleInputLevel').addEventListener('change', function () {
            ...
            if (level === '{{\App\Enums\UserLevelEnum::User}}') {
                ...
                action.innerHTML = despachadorSubMenu;
            } else if (level === '{{\App\Enums\UserLevelEnum::Supervisor}}') {
                action.innerHTML = supervisor;
            }
        });

        document.getElementById('menuForm').addEventListener('change', function () {
            const level = document.getElementById('exampleInputLevel').value;
            const tipoServico = document.getElementById('exampleInputTipoServico');
            const action = document.getElementById('action');

            if (level === '{{\App\Enums\UserLevelEnum::Desp}}' && tipoServico.value === 'radioPatrulhamento') {
                action.innerHTML = radioPatrulhamentoSubMenu;
            } else if (level === '{{\App\Enums\UserLevelEnum::Desp}}' && tipoServico.value === 'especialidades') {
                action.innerHTML = especialidadesSubMenu;
            }
        });
    </script>
@endsection
